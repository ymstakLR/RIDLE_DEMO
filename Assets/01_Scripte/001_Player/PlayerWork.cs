...
public class PlayerWork : MonoBehaviour {
    ...
    private float OperationKeyChange() {
        if(_pJump.IsFlipJumpFall)//左右重力でFlipJumpをして落下する場合
            return ConfigManager.Instance.config.GetAxisRaw(ConfigData.Horizontal.String);
        switch (this.transform.localEulerAngles.z) {
            case 0:
                return ConfigManager.Instance.config.GetAxisRaw(ConfigData.Horizontal.String);
            case 90:
                return ConfigManager.Instance.config.GetAxisRaw(ConfigData.Vertical.String);
            case 180:
                return -ConfigManager.Instance.config.GetAxisRaw(ConfigData.Horizontal.String);
            case 270:
                return -ConfigManager.Instance.config.GetAxisRaw(ConfigData.Vertical.String);
